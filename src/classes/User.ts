...
export class User {
  ...
  public async bookJourney(
    ...
    await globalDatabase.insertNewJourney(
      this._id,
      carId,
      description,
      dateStart,
  ...
  public async viewStatistics(): Promise<void> {
    ...
    const totalAmount = allJournies.reduce(
      (acc, journey) => acc + journey.costAmount,
      0
    );
    console.log("Total sum of fares \t", totalAmount);
    console.log("Amount of fares booked\t", allJournies.length);
    console.log(
      "Average price of fares\t",
      (totalAmount / allJournies.length).toFixed(2)
    );
  }
  ...
  public async viewUpcomingJournies(): Promise<void> {
      ...
      return (
        ...
        Date.parse(
          new Date(
            ...
            journeyDate.getDate(),
            parseInt(journeyTime[0]),
            parseInt(journeyTime[1])
          ).toISOString()
        )
      );
    });
    ...
    if (filtedUpcomingJournies.length == 0) {
      console.log(
        "It seems like you don't have any journies coming up. Book one now!"
      );
      return;
    }
  ...
  public async viewPastJournies(): Promise<void> {
    ...
    if (filtedPastJournies.length == 0) {
      console.log(
        "It seems like you don't have any past journies. Book one now!"
      );
      return;
    }
  ...
  private displayFilteredJournies(filteredJournies: any): void {
    ...
    for (const journey of filteredJournies) {
      ...
      const upcomingJourney = new Journey(
        journey._id,
        journey.userId,
        journey.carId,
        journey.description,
