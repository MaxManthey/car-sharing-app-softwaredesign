...
export class Controll {
  ...
  private async viewCars(): Promise<void> {
    ...
    const response = await prompts({
      ...
      validate: (chosenCar) =>
        chosenCar < 0 || chosenCar > carCounter
          ? `Must be > 0 and <= ${carCounter}`
          : true,
    });
    ...
    if (chosenCarNum == undefined || chosenCarNum == carCounter) {
      ...
      if (JSON.stringify(bookingPreferences).length < 3) {
        ...
        const costAmount: number =
          chosenCar.getFlatFee() + chosenCar.getPricePerMinute() * useTime;

        const carTimeOk = chosenCar.providedDateTimeMatch(
          dateTime.getHours(),
          dateTime.getMinutes(),
          useTime
        );
        const carNotBookedYet = await chosenCar.carNotBookedYet(
          date,
          dateTime.getHours(),
          dateTime.getMinutes(),
          useTime
        ...
        if (!carTimeOk || !carNotBookedYet) {
          const message = !carTimeOk
            ? "The provided details didn't match the cars requirements, please try again"
            : "The car has already been booked, sorry about that!";
          console.log(message);
          return;
        }
        ...
        await this.user.bookJourney(
          chosenCar.getId(),
          chosenCar.getDescription(),
          date,
          hours + ":" + minutes,
          useTime,
          costAmount
  ...
  private async viewFilteredCars(): Promise<void> {
    ...
    const response = await prompts({
      ...
      choices: [
        ...
        {
          title: "Electric",
          description: "Filter for electric cars",
          value: "Electric",
        },
        {
          title: "I don't care",
          description: "Filter for electric and gas cars",
          value: "none",
        },
      ],
    });
    ...
    const filteredCars: any = await globalDatabase.filterCarsByDriveAndDuration(
      driveType,
      bookingPreferences.useTime
    );
    const dateTime: Date = new Date(bookingPreferences.date);
    const date: Date = new Date(
      dateTime.getFullYear(),
    ...
    for (const car of filteredCars) {
      ...
      const carTimeOk = currentCar.providedDateTimeMatch(
        dateTime.getHours(),
        dateTime.getMinutes(),
        useTime
      );
      const carNotBookedYet = await currentCar.carNotBookedYet(
        date,
        dateTime.getHours(),
        dateTime.getMinutes(),
        useTime
    ...
    if (availableCars.length == 0) {
      console.log(
        "There are no available cars, that match your booking preference. Sorry about that!"
      );
      return;
    }
    ...
    for (const car of availableCars) {
      ...
      console.log(
        "-> Total amount to pay: \t",
        car.getFlatFee() + car.getPricePerMinute() * useTime
      );
      console.log();
    }
    console.log("--", ++carCounter, "--");
      ...
      choices: [
        {
          ...
          description: "View cars that are available now",
          value: "viewCars",
        },
        {
          title: "Filter and view cars",
          description: "Filter available cars to your preferance",
          value: "viewFilteredCars",
        },
      ],
    };
    if (!this.user.isNull()) {
      this.userOptions.choices.push({
        title: "View statistics",
        description: "View your statistics",
        value: "viewStatistics",
      });
      this.userOptions.choices.push({
        title: "View upcoming journies",
        description: "View your upcoming journies",
        value: "viewUpcomingJournies",
      });
      this.userOptions.choices.push({
        title: "View past journies",
        description: "View your past journies",
        value: "viewPastJournies",
      });
    }
    if (this.user.getIsAdmin()) {
      this.userOptions.choices.push({
        title: "Add new car",
        description: "Add a new car to the pool",
        value: "addCar",
      });
  ...
  private async confirmBooking(): Promise<boolean> {
    const response = await prompts({
      ...
      choices: [
        {
          title: "Confirm",
          description: "Yes, I would like to book the journey",
          value: true,
        },
        {
          title: "Decline",
          description: "No, I don't want to book the journey",
          value: false,
        },
      ],
    });
