...
export class UserManagement {
  public async login(): Promise<object> {
    ...
    if (providedUsername == undefined || providedPassword == undefined) {
      ...
      !(await globalDatabase.passwordMatching(
        providedUsername,
        providedPassword
      ))
    ) {
  ...
  private async checkUsername(): Promise<string> {
    ...
    if (response.username == undefined) {
      ...
      console.log(
        "The username can only contain letters and numbers and must be at least 2 characters long"
      );
      return await this.checkUsername();
  ...
  private async checkPassword(): Promise<string> {
    ...
    if (response.password == undefined) {
      ...
      !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{4,}$/.test(response.password)
    ) {
      console.log(
        "Your password must contain at least 4 characters, 1 letter and 1 number"
      );
      return await this.checkPassword();
    } else {
