...
export class Database {
  ...
  public async connect(): Promise<boolean> {
    ...
    this.journiesCollection = this.mongoClient
      .db(this.dbName)
      .collection("journies");

    if (
      this.usersCollection == undefined ||
      this.carsCollection == undefined ||
      this.journiesCollection == undefined
    ) {
      return false;
    }
    return true;
  ...
  public async insertNewJourney(
    userId: Mongo.ObjectId,
    carId: Mongo.ObjectId,
    description: string,
    dateStart: Date,
    timeStart,
    duration,
  ...
  public async getJourniesByCarIdAndDate(
    carId: Mongo.ObjectId,
    dateStart: Date
  ): Promise<object> {
    return await this.journiesCollection
      .find({
        carId: carId,
        dateStart: dateStart,
      })
  ...
  public async getTenCarsToDisplay(): Promise<object[]> {
    return await this.carsCollection
      .aggregate([{ $sample: { size: 10 } }])
      .toArray();
  }

  public async filterCarsByDriveAndDuration(
    drive: string,
    duration: number
  ): Promise<object[]> {
    if (drive == "none") {
      return await this.carsCollection
        .find({ maxUseTime: { $gte: duration } })
        .toArray();
    }
    return await this.carsCollection
      .find({
